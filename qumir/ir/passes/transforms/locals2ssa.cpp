                ...
                switch (instr.Op) {
                    case "stre"_op: {
                        ...
                        if (localIdx >= Function.ArgLocals.size() &&
                            ...
                            if (IsStructLocal(localIdx) && !IsStructValueOperand(instr.Operands[1])) {
                                // Pointer-backed struct stores stay in memory.
                                break;
                            }
