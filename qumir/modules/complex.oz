(block
  ...
  (fun i () -> компл
    (block (return (: (struct ((re (: 0.0 f64)) (im (: 1.0 f64)))) компл))))
  ...
  (fun сопряжённое ((var z компл)) -> компл
    (block (return (: (struct ((re (field z re)) (im (- (: 0.0 f64) (field z im))))) компл))))
  ...
  (fun complex_neg ((var a компл)) -> компл (attrs (operator "neg"))
    (block (return (: (struct ((re (- (: 0.0 f64) (field a re))) (im (- (: 0.0 f64) (field a im))))) компл))))
  ...
  (fun __imag ((var x f64)) -> компл (attrs (literal "i"))
    (block (return (: (struct ((re (: 0.0 f64)) (im x))) компл))))

  (fun complex_from_float ((var x f64)) -> компл (attrs (operator "cast"))
    (block (return (: (struct ((re x) (im (: 0.0 f64)))) компл))))
  (fun complex_from_int ((var x i64)) -> компл (attrs (operator "cast"))
    (block (return (: (struct ((re (cast x f64)) (im (: 0.0 f64)))) компл))))
