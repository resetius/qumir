...
- `длин(s)` — длина строки
- `s[i]` — символ по индексу (нумерация с 1)
- `s[i:j]` — подстрока от i до j включительно
...
- `юникод(символ)` — получить код символа
- `юнисимвол(код)` — создать строку из кода
...
- `поз(строка, образец)` — позиция первого вхождения (0 если не найдено)
- `поз после(начало, строка, образец)` — поиск после указанной позиции
...
[▶ Запустить пример](/?example=strings.kum)
