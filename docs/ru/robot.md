...
В онлайн-песочнице команды движения (`влево`, `вправо`, `вверх`, `вниз`) и команда `закрасить` являются шагами анимации. После такой команды управление возвращается интерфейсу, поле перерисовывается, а выполнение продолжается после небольшой задержки.

Благодаря этому рекурсивные алгоритмы и циклы видны по шагам. Бесконечная программа тоже не должна подвешивать вкладку, пока внутри цикла регулярно выполняются команды Робота.
